...
public class PlayerCookingSystem : MonoBehaviour
{
    ...
    private Renderer[] allRenderers;   // <– ahora todos, no solo un
    ...
    void Start()
    {
        ...
        // guardamos alguno por si falta material
        if (allRenderers != null && allRenderers.Length > 0)
        {
            originalMaterialsPerRenderer = new Material[allRenderers.Length][];
            for (int i = 0; i < allRenderers.Length; i++)
            {
                // clonamos la lista de materiales de ese renderer
                originalMaterialsPerRenderer[i] = allRenderers[i].materials;
            }
        }

        UpdateMultipliers(); // Inicializa baseSpeed/baseJump/baseDamage según estado
        UpdateVisuals();
        Debug.Log($"[PlayerCookingSystem] Start con estado: {currentState}");
    }
    void Update()
    {
    ...
    private void UpdateVisuals()
    {
        ...
        {
            case PlayerFoodState.Normal:
                // si NO pusiste material para Normal -> restaurar los originales
                if (normalMaterial == null)
                {
                    // restaurar cada renderer con su lista de materiales original
                    if (originalMaterialsPerRenderer != null)
                    {
                        for (int i = 0; i < allRenderers.Length; i++)
                        {
                            if (originalMaterialsPerRenderer[i] != null)
                                allRenderers[i].materials = originalMaterialsPerRenderer[i];
                        }
                    }
                    Debug.Log("🎨 Restaurado al diseño original (Normal sin material).");
            ...
            case PlayerFoodState.Cooked:
                targetMat = cookedMaterial != null ? cookedMaterial : null;
                break;

            case PlayerFoodState.Fried:
                targetMat = friedMaterial != null ? friedMaterial : null;
                break;

            case PlayerFoodState.Boiled:
                targetMat = boiledMaterial != null ? boiledMaterial : null;
                break;

            case PlayerFoodState.Burned:
                targetMat = burnedMaterial != null ? burnedMaterial : null;
                break;
        }

        // si el estado sí tenía material, lo aplicamos a todos
        if (targetMat != null)
        {
            for (int i = 0; i < allRenderers.Length; i++)
            {
            ...
            Debug.Log($"🎨 Material cambiado a: {currentState}");
        }
        else
        {
            // si llegas aquí es que ese estado no tenía material asignado,
            // podemos dejar los que ya tenía puestos o restaurar los originales
            Debug.Log($"⚠️ Estado {currentState} no tiene material asignado.");
        }
    }
